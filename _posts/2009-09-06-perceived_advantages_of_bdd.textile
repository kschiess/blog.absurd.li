...
don't write specs have also made these choices. _But only mentally_. And then
forgot about it before lunch. It doesn't feel differently, the code commits
might even be exactly the same. Except, some time later, you (the guy who does
BDD) will still have a record of what you thought of (or didn't) when writing
the code.

I had to maintain code that looked like this once: 

<pre><code class="sh_java">
...
the whole exception and wait for things that break or leave it in and hope
someone else breaks it later. Either way, this is a liability that waits to
bite you the next time around. Having to remember that functions with a
positive domain # TODO may return a special negative value that indicates
mystery going on makes reasoning about the system a complex task.
